...
query="CREATE TABLE IF NOT EXISTS sale (custName VARCHAR(20), date VARCHAR(10),Ph INTEGER, Amount INTEGER )" 
my_cursor.execute(query) #executing the query
...
def bill():
    ...
    Quit = Button(wn,text="Quit",bg='#f7f1e3', fg='black', command=wn.destroy)
    Quit.place(relx=0.53,rely=0.9, relwidth=0.18,relheight=0.08)
    #Checking if the quantity of the 0rd product is entered and calculating price, showing it on window  and adding to database 
    if len(res) == 0:
          Label(labelFrame, text="no product in the data base",font = ('calibri',11,'bold'),fg='black').place(relx=0.3,rely=0.5)
    #Checking if the quantity of the 1st product is entered and calculating price, showing it on window  and adding to database
    #here the if condition is used so that if we enter 1 item or upto 5 items the bill will be generated
    if len(res) == 1:
         if(len(name1.get()) != 0):
                 i=res[0]
                 qty=int(name1.get())
                 total=qty*int(i[2])
                 Label(labelFrame,text="%-40s%-50s%-50s%-50s"%(i[1],i[2],qty,total) ,fg='black').place(relx=0.07,rely=y)
                 totalBill+=total
                 y+=0.1
    ...
    #Checking if the quantity of the 4rd product is entered and calculating price, showing it on window  and adding to database              
    if len(res) == 4:  
        if(len(name1.get()) != 0):
                 i=res[0]
                 qty=int(name1.get())
    ...
    #Checking if the quantity of the 5rd product is entered and calculating price, showing it on window  and adding to database 
    if len(res) == 5:
        if(len(name1.get()) != 0):
                 i=res[0]
                 qty=int(name1.get())
    ...
    Label(labelFrame,text="TOTAL PRICE:"+"\t\t\t\t\t\t\t\t"+str(totalBill) ,fg='black').place(relx=0.07,rely=y)
...
def newCust():    
    global wn,name1,name2,name3,name4,name5,date,custName,Ph
    #Creating a window
    wn = tkinter.Tk() 
    wn.title("SHOP EASY Management System")
    wn.configure(bg='lavender blush2')
    ...
    lable2.place(relx=0.05, rely=0.37)
      
    # Getting customer name
    custName = Entry(wn)
    custName.place(relx=0.3, rely=0.37, relwidth=0.62)
    
    # Getting customer phone number
    lable3 = Label(wn, text="Phone No:", fg='black')
    lable3.place(relx=0.05, rely=0.44)
    Ph = Entry(wn)
    Ph.place(relx=0.3, rely=0.44, relwidth=0.62)
    ...
    if len(res) == 0:
                     Label(labelFrame, text="NO PRODUCT IS ADDED",font = ('calibri',11,'bold'),fg='black').place(relx=0.3,rely=0.5)
...
def totsale():
    ...
    headingLabel = Label(headingFrame1, text="TOTAL SALES", fg='black', font = ('Courier',15,'bold'))
    headingLabel.place(relx=0,rely=0, relwidth=1, relheight=1)
    ...
    Label(labelFrame, text="%-20s%-30s%-20s%-20s" % ('CustName', 'date', "Ph", "totalBill"), font=('calibri', 11, 'bold'), fg='black').place(relx=0.07, rely=0.1)
    ...
    try:
        ...
        for i in res:
            Label(labelFrame,text="%-20s%-30s%-20s%-20s" % (i[0],i[1],i[2],i[3]) ,fg='black').place(relx=0.07,rely=y)
            y += 0.1
    except Exception as e:
        print("The exception is:",e)
        messagebox.showinfo("Failed to fetch files from database")
...
wn.minsize(width=500,height=500)
wn.maxsize(width=700,height=600)
wn.geometry("700x600")
...
btn4.place(x=250,y=415)

#button to see the total sales
btn4 = Button(wn,text="Total Sale",bg='lavender blush2', fg='black', width=20,height=2,command = totsale)
btn4['font'] = font.Font( size=12)
btn4.place(x=250,y=490,)
